...
(function () {
  ...
  /* Kho trên Supabase đã tách theo môn qua ns() ở trên, nhưng sổ còn giữ VÀI thứ ngay trong máy —
     localStorage khoá cố định (sbt-bank-v1, sbt-weeks-v1, sbt-settings-v1, …) — vốn viết thẳng
     trong mã sổ (bản Artifact chạy riêng từng origin nên không sao), còn mọi bản web lại chạy
     CHUNG một tên miền giangduonghoahoc.com → trình duyệt coi đó là MỘT localStorage duy nhất.
     Mở sổ Hữu cơ sau khi vừa dùng sổ gốc thì thấy nguyên kho câu của sổ gốc do đọc trúng cùng
     khoá. Gắn thêm tiền tố NS vào riêng các khoá "sbt-…" của app; khoá "sb-...-auth-token" mà
     supabase-js tự quản lý cho phiên đăng nhập KHÔNG đổi, để đăng nhập một lần dùng chung được
     cho mọi môn. (IndexedDB 'keyval-store' của Tesseract.js — kho tệp ngôn ngữ OCR — CỐ Ý không
     tách: chỉ là dữ liệu mô hình OCR dùng chung, tách ra chỉ tốn chỗ tải lại cho từng môn.) */
  if (NS) {
    var lsGet = localStorage.getItem.bind(localStorage),
        lsSet = localStorage.setItem.bind(localStorage),
        lsDel = localStorage.removeItem.bind(localStorage);
    function lsKey(k) { return /^sbt-/.test(k) ? NS + k : k; }
    localStorage.getItem = function (k) { return lsGet(lsKey(k)); };
    localStorage.setItem = function (k, v) { return lsSet(lsKey(k), v); };
    localStorage.removeItem = function (k) { return lsDel(lsKey(k)); };
  }
  ...
  function badge(name, email) {
    ...
    if (!b) {
      ...
      menu.innerHTML =
        ...
        (NS ? '<button type="button" id="sbtDonKho">🧹 Dọn câu dính từ Hóa phân tích</button>' : '') +
        '<a href="quan-tri.html">▦ Trang quản trị lớp</a>' +
        '<a href="hoc.html" target="_blank" rel="noopener">👁 Trang học (xem như sinh viên)</a>' +
      ...
      if (NS) menu.querySelector('#sbtDonKho').addEventListener('click', function () { menu.hidden = true; donKhoDinh(); });
      mountNavGiao();
  ...
  /* Dọn câu/thùng-rác bị "cứu" NHẦM từ Hóa phân tích sang môn khác (sửa 16/9/2026). Trước khi
     localStorage được tách theo môn (đoạn vá lsKey phía trên), mở sổ môn khác trên máy đã dùng sổ
     gốc trước đó từng đọc trúng kho câu chung của trình duyệt — rồi cơ chế "câu chỉ có trên máy thì
     đẩy lên ngay" (đợt 63 của app) tưởng đó là câu riêng của môn này nên ĐẨY THẬT lên kho trên
     Supabase của môn này, không chỉ hiện sai trên màn hình. Cùng một mã câu (id tự sinh theo giờ +
     số ngẫu nhiên, không thể trùng tự nhiên) xuất hiện ở CẢ kho gốc (không tiền tố) VÀ kho môn này
     là dấu hiệu chắc chắn của lần đẩy nhầm đó. Chỉ xoá ở kho MÔN NÀY sau khi giáo viên xác nhận,
     không đụng tới kho gốc. */
  async function laySoDocId(collection) {
    var r = await sb.from(TABLE).select('doc_id, data').eq('collection', collection);
    if (r.error) throw new Error(r.error.message);
    return r.data || [];
  }
  async function donKhoDinh() {
    if (!NS) return;
    try {
      var goc = await laySoDocId('bank'), rieng = await laySoDocId(ns('bank'));
      var gocT = await laySoDocId('trash'), riengT = await laySoDocId(ns('trash'));
      var gocIds = {}; goc.forEach(function (d) { gocIds[d.doc_id] = true; });
      var gocTIds = {}; gocT.forEach(function (d) { gocTIds[d.doc_id] = true; });
      var dinhBank = rieng.filter(function (d) { return gocIds[d.doc_id]; });
      var dinhTrash = riengT.filter(function (d) { return gocTIds[d.doc_id]; });
      if (!dinhBank.length && !dinhTrash.length) {
        alert('Không thấy câu nào dính từ Hóa phân tích. Kho của ' + MON.name + ' đang sạch.');
        return;
      }
      var mau = dinhBank.slice(0, 5).map(function (d) {
        var t = d.data && d.data.questionHtml ? String(d.data.questionHtml).replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim() : '';
        return '· ' + (t ? t.slice(0, 70) : d.doc_id);
      }).join('\n');
      var hoi = 'Thấy ' + dinhBank.length + ' câu' + (dinhTrash.length ? ' và ' + dinhTrash.length + ' mục thùng rác' : '')
        + ' trong kho ' + MON.name + ' trùng mã với kho Hóa phân tích — gần như chắc chắn là câu bị đẩy nhầm sang trước đây.\n\n'
        + (mau ? 'Vài câu ví dụ:\n' + mau + '\n\n' : '')
        + 'Xoá số câu này khỏi kho ' + MON.name + '? Kho Hóa phân tích KHÔNG bị đụng tới.\n'
        + '(Nếu câu nào trong đây thật ra do m tự soạn riêng cho ' + MON.name + ' mà trùng đúng mã — rất khó xảy ra — thì bấm Huỷ rồi báo lại.)';
      if (!confirm(hoi)) return;
      var okB = 0, okT = 0;
      for (var i = 0; i < dinhBank.length; i++) {
        var r1 = await sb.from(TABLE).delete().eq('collection', ns('bank')).eq('doc_id', dinhBank[i].doc_id);
        if (!r1.error) okB++;
      }
      for (var j = 0; j < dinhTrash.length; j++) {
        var r2 = await sb.from(TABLE).delete().eq('collection', ns('trash')).eq('doc_id', dinhTrash[j].doc_id);
        if (!r2.error) okT++;
      }
      alert('Đã xoá ' + okB + '/' + dinhBank.length + ' câu' + (dinhTrash.length ? ' và ' + okT + '/' + dinhTrash.length + ' mục thùng rác' : '')
        + ' khỏi kho ' + MON.name + '. Trang sẽ tải lại để thấy đúng.');
      location.reload();
    } catch (e) {
      alert('Không dọn được: ' + (e && e.message || e));
    }
  }
  ...
  /* ---- xếp hàng ghi: tối đa 4 lệnh một lúc, rớt thì thử lại 4 lần ----
     Khôi phục từ file sao lưu bắn cả trăm lệnh cùng lúc; không xếp hàng thì Supabase chặn bớt
     và app nuốt lỗi, mất dữ liệu mà không báo. */
  var QMAX = 4, qRunning = 0, qWait = [], qFail = 0, qDone = 0, qNoteTimer = null;
  function qSlot() {
    if (qRunning < QMAX) { qRunning++; return Promise.resolve(); }
    return new Promise(function (res) { qWait.push(res); });
  }
  function qRelease() {
    qRunning--;
    var next = qWait.shift();
    if (next) { qRunning++; next(); }
  }
  function qReport() {
    clearTimeout(qNoteTimer);
    qNoteTimer = setTimeout(function () {
      if (qFail > 0) {
        alert('Có ' + qFail + ' mục không ghi được lên kho (mạng chập chờn). Hãy làm lại thao tác vừa rồi, '
          + 'ví dụ khôi phục lại từ file sao lưu — ghi lại đè lên là an toàn, không nhân đôi.');
        qFail = 0;
      }
      qDone = 0;
    }, 2500);
  }
  async function qRun(fn) {
    await qSlot();
    try {
      var lastErr = null;
      for (var i = 0; i < 4; i++) {
        try {
          ...
          lastErr = e;
          await new Promise(function (res) { setTimeout(res, 250 * Math.pow(2, i)); });
        }
      }
      qFail++; qReport();
      throw lastErr;
    ...
    return {
      ...
      set: function (data) {
        ...
        return qRun(function () {
          return sb.from(TABLE).upsert(body, { onConflict: 'owner,collection,doc_id' });
        }).then(function () {});
      },
      ...
      delete: function () {
        return qRun(function () {
          return sb.from(TABLE).delete().eq('collection', collection).eq('doc_id', id);
        }).then(function () {});
      },
  ...
  /* Cột trái của sổ dựng sau khi app khởi động, nên chờ nó xuất hiện rồi mới cắm nút vào
     nhóm "Xuất" — cạnh In / Xuất PDF, đúng chỗ tay người ta tìm tới khi muốn phát tài liệu. */
  function mountNavGiao() {
    var lan = 0;
    (function thu() {
      if (document.getElementById('sbtNavGiao')) return;
      var nhom = null;
      var gs = document.querySelectorAll('.sn-group');
      for (var i = 0; i < gs.length; i++) {
        var h = gs[i].querySelector('h4');
        if (h && h.textContent.trim() === 'Xuất') { nhom = gs[i]; break; }
      }
      if (!nhom) { if (++lan < 40) setTimeout(thu, 300); return; }
      var b = document.createElement('button');
      b.type = 'button';
      b.id = 'sbtNavGiao';
      b.className = 'sn-item sn-primary';
      b.textContent = '📤 Giao cho lớp';
      b.title = 'Đẩy thẳng tài liệu đang mở lên lớp cho sinh viên xem — không tải tệp nào về máy';
      b.addEventListener('click', openSend);
      nhom.insertBefore(b, nhom.querySelector('h4').nextSibling);
    })();
  }
